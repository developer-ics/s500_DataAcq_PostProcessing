...
def load_yellowfin_NMEA_files(fpath, saveFname, plotfname=False, verbose=False):
    """loads and possibly plots NMEA data from yellowfin
    ...
    with h5py.File(saveFname, 'w') as hf:
        ...
        hf.create_dataset('pc_time_gga', data=[datetime_to_epoch(pc_time_gga[ii]) for ii in range(len(pc_time_gga))])
        hf.create_dataset('gps_time', data=[datetime_to_epoch(aa[i]) for i in range(len(aa))][0] )
...
fpath = '/data/yellowfin/20230327/nmeaData'
saveFname = '/data/yellowfin/20230327/gnss_raw.h5'
