...
int_pro=0; % this variabe enables processing of the full intensity profile, set to zero just uses the S500 estaimate of bed location
...
dirstr='04-14-2023' % alos with emlid ppk GPS saved and processed
...
for fi=1:length(dd) %5  20201218-101925265.bin ios new 500 khz
    ...
    for ii=1:(length(st)-2)
        ...
        % 1300 packet are only produced by ping sonar from bluerobotics
...
%%
dirstr='04-14-2023' 
load([ 'detected_range' , dirstr],  'dt_txt' ,'txt_smooth_depth', 'txt_depth'); 
figure(7);clf
plot(dt_txt,txt_depth,'b');
hold on
plot(dt_txt(1:end)+(dt_txt(1)-dt_txt(10)),txt_smooth_depth(1:end),'r')% note delay of 10 samples to account for smoothing filter
