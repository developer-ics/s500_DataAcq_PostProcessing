...
%% Get beach rover data if option selected
if include_beach_rover
    BR=readtable(beach_rover_data_path)
end
%%
...
%%
geoid_height=nanmean(GPS.GeoidSeparation);
gpsantenna_offset_waterline=.15;
sonar_waterline_offset=.10;%sonar to waterline
x=PPKGPS.lon_ppk;
y=PPKGPS.lat_ppk;
z=-shifted_sonar_range_i(:)+PPKGPS.clean_height_datum_ppk-(sonar_waterline_offset+gpsantenna_offset_waterline)-0*geoid_height;
%
ppk_gnss_time_utc=PPKGPS.datetime_ppk_g(:);
longitude=x(:);latitude=y(:);
z_water_surf=-(shifted_sonar_range_i(:) +sonar_waterline_offset(:));
z_gps_datum=PPKGPS.clean_height_datum_ppk(:)-0*geoid_height;
z_seafloor_datum=z_water_surf(:)  + z_gps_datum(:) - sonar_waterline_offset(:);
ppk_gnss_time_utc=datetime(ppk_gnss_time_utc,"Format",'yyyy-MM-dd hh:mm:ss.SSS')
To=table(ppk_gnss_time_utc,longitude,latitude,z_seafloor_datum,z_water_surf,z_gps_datum);
writetable(To,[odir 'PPK_Heave_Corrected_Trackline_Data' fs2 '.txt']);

save([odir 'PPK_Heave_Corrected_Trackline_Data' fs2 '.mat'],'To');
if include_beach_rover
    x=[x(:); xb(:)]; 
    y=[y(:); yb(:)]; 
    z=[z(:); zb(:)]; 
...
if make_maps_ppk
  ...
  colormap(cm)
   % caxis([prctile(fillmissing(z,'linear'),1) prctile(fillmissing(z,'linear'),99) ])
        caxis([prctile(z,1) prctile(z,99) ])
    ...
    %     pause(.1)
    % end
    k=boundary(x(:),y(:),.1);%x and y are track line coordinate vectors in UTM, z is the surface re datum
