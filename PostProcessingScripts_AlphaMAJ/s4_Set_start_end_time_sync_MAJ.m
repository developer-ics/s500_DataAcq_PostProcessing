...
time_shift=round(24*(datenum(GPS.pc_time_gga(1000))-datenum(GPS.gps_utc_time(1000))));
...
subplot(312)
sonar_time_rp_utc=datetime(datetime(sonar_time_rp),'TimeZone','UTC');
...
stind=30;
start_ind_gps=find(GPS.altMSL(stind:end)<GPS_thresh,1,'first');
start_time_gps=GPS.gps_utc_time(stind+start_ind_gps)+seconds(30); %offset till the boat is in the water
...
sonar_mtime_utc_gps=polyval(pp,datenum(datetime(sonar_time_rp,'TimeZone','UTC')),[],mu);
